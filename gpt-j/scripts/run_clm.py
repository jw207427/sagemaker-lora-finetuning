...
def parse_arge():
    ...
    parser.add_argument(
        ...
        help="Location of the model used for fine tuning",
    )
    parser.add_argument(
        "--token_path",
        type=str,
        default="/opt/ml/input/token",
...
def create_peft_config(model):
    ...
    peft_config = LoraConfig(
        ...
        lora_alpha=32,
        lora_dropout=0.05,
        target_modules=["q_proj","v_proj"], # Bloom target_modules=["query_key_value"],
    )
...
def training_function(args):
    ...
    print(f"check files availble in {path}: ============ ")
    print(os.listdir(path))
    
    print(f"check files availble in {args.model_path}: ============ ")
    ...
    shutil.copyfile(
        ...
        f"{args.model_dir}/code/inference.py",
    )
    shutil.copyfile(
        os.path.join(os.path.dirname(__file__), "requirements.txt"),
        f"{args.model_dir}/code/requirements.txt",
    )
